...
class Minesweeper:
    def __init__(self, mines:Mines, screen, reset_btn, x_screen, y_screen):
        self.screen = screen
        self.reset_button = reset_btn
        self.clock = pygame.time.Clock()
        self.mines = mines
        self.load_sounds()
        self.reset_game(EASY,x_screen,y_screen)
        
    def load_sounds(self):
        self.explosion_sound = pygame.mixer.Sound('src/minesweeper/assets/audio/squeaky.mp3')
        ...
        self.loser_sound = pygame.mixer.Sound('src/minesweeper/assets/audio/loser.mp3')
    
    def reset_game(self, level, x_screen, y_screen):
        self.grid = self.create_grid(level,x_screen, y_screen)
    ...
    def create_grid(self, level, x_screen, y_screen):
        grid = []
    ...
    def handle_click(self, square):
        if square["is_a_mine"] and square["is_revealed"]:
            ...
            self.game_over = True
            # add un delay pour jouer le son loser et afficher perdu
    ...
    def run(self,WIDTH_SCREEN, HEIGHT_SCREEN):
        running = True
        while running:
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if self.reset_button.collidepoint(event.pos):
                        self.reset_game(EASY,WIDTH_SCREEN, HEIGHT_SCREEN)
                        self.reset_sound.play()
            
            self.draw_grid(WIDTH_SCREEN)
            self.clock.tick(30)
